...
class VideoMix(object):
    ...
    def __init__(self):
        ...
        self.log.info(
            'Configuring mixer for %u source(s) and %u background source(s)',
            len(self.sources),
            len(self.bgSources),
        )
    ...
    def attach(self, pipeline):
        ...
        self.bgScene = Scene(
            self.bgSources, pipeline, self.transitions.fps, 0, cropping=False
        )
        self.scene = Scene(
            self.sources, pipeline, self.transitions.fps, len(self.bgSources)
        )
        self.compositeMode = None
        self.sourceA = None
        self.sourceB = None
        self.setCompositeEx(
            Composites.targets(self, self.composites)[0].name,
            self.sources[0],
            self.sources[1],
        )
    ...
    def setCompositeEx(
        ...
        if not self.compositeMode:
            ...
            curB = self.sourceB
            self.log.info(
                "Request composite change from %s(%s,%s) to %s(%s,%s)",
                curCompositeName,
                curA,
                curB,
                newCompositeName,
        ...
        if curCompositeName and not (newCompositeName and newA and newB):
            ...
            self.log.debug(
                "Completing wildcarded composite to %s(%s,%s)",
                newCompositeName,
                newA,
                newB,
            )
        ...
        if newComposite and newA in self.sources and newB in self.sources:
            self.log.debug(
                "New composite shall be %s(%s,%s)", newComposite.name, newA, newB
            )
            ...
            if useTransitions:
                if curComposite:
                    ...
                    if len(set(old)) == 3:
                        self.log.debug(
                            "Current composite includes three different frames: (%s,%s) -> (%s,%s)",
                            *old
                        )
                        # check if current composite hides B
                        if curComposite.single():
                            self.log.debug(
                                "Current composite hides channel B so we can secretly change it."
                            )
                            # check for (A,B) -> (A,C)
                            if curA == newA:
                                # change into (A,C) -> (A,C)
                            ...
                            # check another case where new composite also hides B
                                ...
                                self.log.debug(
                                    "New composite also hides channel B so we can secretly change it."
                                )
                    ...
                    if len(set(old)) == 4:
                        self.log.debug(
                            "Current composite includes four different frames: (%s,%s) -> (%s,%s)",
                            *old
                        )
                        # check if both composites hide channel B
                        if curComposite.single() and newComposite.single():
                            self.log.debug(
                                "Current and new composite hide channel B so we can secretly change it."
                            )
                    ...
                    if old != (curA, curB, newA, newB):
                        self.log.info(
                            "Changing requested transition from (%s,%s) -> (%s,%s) to (%s,%s) -> (%s,%s)",
                            *old,
                            curA,
                            curB,
                            newA,
            ...
            if transition:
                ...
                self.log.debug("committing transition '%s' to scene", transition.name())
            ...
            if curBgSource != newBgSource:
                ...
                if transition:
                    ...
                    curBgFrame = Frame(
                        True, zorder=0, rect=[0, 0, *Config.getVideoResolution()]
                    )
                    ...
                    newBgFrame = Frame(
                        ...
                        alpha=0,
                        zorder=1,
                        rect=[0, 0, *Config.getVideoResolution()],
                    )
                    self.bgScene.commit(
                        newBgSource, fade_alpha(newBgFrame, 255, transition.frames())
                    )
                else:
                    # apply new scene (hard cut)
    ...
    def testCut(self, command):
        ...
        command = CompositeCommand.from_str(command)
        if (
            command.composite != self.compositeMode
            or command.A != self.sourceA
            or command.B != self.sourceB
        ):
    ...
    def testTransition(self, command):
        ...
        return self.setCompositeEx(command.composite, command.A, command.B, True, True)
