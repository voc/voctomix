...
def parse():
    ...
    parser.add_argument(
        '-i', '--ini-file', action='store', help="Load a custom config.ini-File"
    )

    parser.add_argument(
        '-p', '--pipeline', action='store_true', help="Generate text files of pipelines"
    )

    parser.add_argument(
        '-n', '--no-bins', action='store_true', help="Do not use gstreamer bins"
    )
