    ...
    def __init__(self, audio_box, source, uibuilder):
        ...
        if source in Config.getSources():
            ...
            for name, stream in self.audio_streams.items():
                self.audio_levels[name] = self.createAudioLevel(name, audio_box, uibuilder)
        else:
            self.audio_level = self.createAudioLevel(source, audio_box, uibuilder)

    def createAudioLevel(self, name, audio_box, uibuilder):
        ...
        self.init_volume_slider(name, audio, uibuilder)
        return uibuilder.find_widget_recursive(audio, 'audio_level_display')
    ...
    def callback(self, rms, peak, decay):
        if self.audio_streams:
            for name, streams in self.audio_streams.items():
                ...
                self.audio_levels[name].level_callback(_rms, _peak, _decay)
        elif self.audio_level:
            self.audio_level.level_callback(rms, peak, decay)

    def init_volume_slider(self, name, audio_box, uibuilder):
